    ...
    var body: some View {
        ...
        .onAppear {
            GenericObserver.shared.observePublisher(
                publisher: AuthManager.shared.loginResultPublisher
                    .eraseToAnyPublisher(),
                ...
                DispatchQueue.main.async {
                    ...
                    case .success(let user):
                        ...
                        print(
                            "Login succeeded for user: \(userLogin?.userName ?? "")"
                        )
            ...
            //update profile info
            GenericObserver.shared.observePublisher(
                publisher: AuthManager.shared.updateProfilePublisher
                    .eraseToAnyPublisher(),
                ...
                DispatchQueue.main.async {
                    ...
                    case .success(let user):
                        self.userLogin = user
//                        self.state = GoPlaySample.updateUserInfo
                        //navigationManager.popToRoot()
        ...
        .onDisappear {
            GenericObserver.shared.cancelSubscriptionByID(
                for: GoPlayAction.loginResult
            )
            GenericObserver.shared.cancelSubscriptionByID(
                for: GoPlayAction.openUpdateProfile
            )
        }
        ...
        NavigationStack {
            List {
                if let userLogin = userLogin {
                    ...
                    if userLogin.accountType != AccountType.apple.rawValue {
                        GoNavigationLink(
                            text: "Update Account",
